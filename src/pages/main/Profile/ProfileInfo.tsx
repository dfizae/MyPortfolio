    ...
    return(
        <>
            <div className="w-full h-full flex flex-col justify-center text-left gap-6">
                <p className="font-bold text-2xl md:text-4xl border-b-3 border-black pb-6">
                    {MESSAGE.NAME}
                </p>
                <ul className="flex flex-col gap-3">
                    {messages.map((message, i) => (
                        <li key={i} className="text-xl md:text-3xl font-normal whitespace-nowrap leading-[1.35]">
                            {message}
                        </li>
                    ))}
                </ul>
